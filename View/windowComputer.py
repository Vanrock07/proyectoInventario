...
class ComputerDataWindow:  
    ...
    def _setup_ui_(self):
        ...
        ttk.Entry(self.frame, width=30, textvariable=self.activo).grid(column=0, row=10, pady=(0, 10), sticky='w')
         ...
         # Tipo de disco
        ttk.Label(self.frame, text="Tipo de disco").grid(column=1, row=1, pady=(0, 5), sticky='w')
        ttk.Entry(self.frame, width=15, textvariable=self.disco).grid(column=1, row=2, pady=(0, 10), sticky='w')
        
         # Tamaño de disco
        ttk.Label(self.frame, text="Tamaño del disco").grid(column=1, row=3, pady=(0, 5), sticky='w')
        ttk.Entry(self.frame, width=15, textvariable=self.tamano).grid(column=1, row=4, pady=(0, 10), sticky='w')
        
         # Ram
        ttk.Label(self.frame, text="Memoria RAM").grid(column=1, row=5, pady=(0, 5), sticky='w')
        ttk.Entry(self.frame, width=15, textvariable=self.ram).grid(column=1, row=6, pady=(0, 10), sticky='w')
        
         # procesador
        ttk.Label(self.frame, text="Procesador").grid(column=1, row=7, pady=(0, 5), sticky='w')
        ttk.Entry(self.frame, width=25, textvariable=self.procesador).grid(column=1, row=8, pady=(0, 10), sticky='w')
